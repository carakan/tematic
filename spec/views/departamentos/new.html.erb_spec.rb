...
describe "/departamentos/new.html.erb" do
  include DepartamentosHelper
  
  before(:each) do
    @departamento = mock_model(Departamento)
    @departamento.stub!(:new_record?).and_return(true)
  ...
  it "should render new form" do
    render "/departamentos/new.html.erb"
    
    response.should have_tag("form[action=?][method=post]", departamentos_path) do
